...
function fetchLikes() {
  axios
    ...
    .then((res) => {
      if (res.status == 200) {
        likeProducts.value = res.data.data;
        console.log(likeProducts.value);
      } else {
        console.error("Xatolik yuz berdi status 200 emas");
      }
    })
...
<template>
  <section class="w-full">
    ...
    >
    <div :class="isloading ? 'h-[70vh] flex flex-col items-center justify-center' : 'hidden'">
        <LoadingPage />
    </div>
  ...
  >
    ...
    >
      ...
      >
        Yoqtirilganlar | {{ likeProducts?.length ? likeProducts?.length : 0 }}
      </button>
    </span>
  </div>
      <div>
      ...
      >
        ...
        >
          ...
          <img class="w-full" src="https://i.pinimg.com/736x/19/6e/86/196e86843d9e8caaafc92b08af1ea7b9.jpg" alt="Kombo" />
          <RouterLink to="/">
